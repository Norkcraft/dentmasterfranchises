...
const serviceGallery: Record<string, { before: StaticImageData; after: StaticImageData; label: string; labelEs: string }[]> = {
  "paintless-dent-repair": [
    { before: beforeDoor, after: afterDoor, label: "Door Dent Repair", labelEs: "Reparación de Puerta" },
    { before: beforeHood, after: afterHood, label: "Hood Dent Repair", labelEs: "Reparación de Capó" },
  ],
  "hail-damage-repair": [
    { before: beforeBack, after: afterBack, label: "Rear Panel Hail Repair", labelEs: "Reparación de Granizo Panel Trasero" },
    { before: beforeHood, after: afterHood, label: "Hood Hail Repair", labelEs: "Reparación de Granizo Capó" },
  ],
  "minor-dent-ding-removal": [
    { before: beforeDoor, after: afterDoor, label: "Door Ding Removal", labelEs: "Eliminación de Golpe de Puerta" },
    { before: beforeBack2, after: afterBack2, label: "Trunk Ding Removal", labelEs: "Eliminación de Golpe de Maletero" },
  ],
  "collision-repair": [
    { before: beforeBack2, after: afterBack2, label: "Rear Collision Repair", labelEs: "Reparación de Colisión Trasera" },
    { before: beforeBack, after: afterBack, label: "Panel Collision Repair", labelEs: "Reparación de Colisión de Panel" },
  ],
  "fender-repair": [
    { before: beforeDoor, after: afterDoor, label: "Fender Dent Repair", labelEs: "Reparación de Guardafangos" },
    { before: beforeBack2, after: afterBack2, label: "Rear Fender Repair", labelEs: "Reparación de Guardafangos Trasero" },
  ],
};

const MiniSlider = memo(function MiniSlider({ before, after, label, t }: { before: StaticImageData; after: StaticImageData; label: string; t: (en: string, es: string) => string }) {
...
export default function ServicePage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (killed || !sectionsRef.current) return;
      ...
      <article className="section-padding bg-background">
        <div className="section-container max-w-4xl" ref={sectionsRef}>
          ...
          <div className="bg-accent/50 rounded-xl p-6 mb-12 hover:shadow-lg transition-shadow duration-300">
            ...
            <Link href="/dealerships-fleet" className="text-sm font-semibold text-primary mt-3 inline-block hover:underline">
              {t("See Fleet Discounts • Save More →", "Ver Descuentos • Ahorre Más →")}
            </Link>
          </div>
          ...
          <div className="mb-12">
            ...
            <div className="flex flex-wrap gap-2">
              {cities.slice(0, 10).map(c => (
                <Link key={c.slug} href={`/service-areas/${c.slug}`} className="text-sm px-3 py-1.5 rounded-full bg-muted text-primary hover:bg-primary hover:text-primary-foreground transition-colors duration-200">{c.city}, FL</Link>
              ))}
            </div>
          </div>
