...
const cityBase = (city: string, slug: string): CityData => {
  const cs = `${city}, FL`;
  const primaryKeyword = `paintless dent repair ${city}`;
  return {
    slug,
    city,
    state: "FL",
    metaTitle: `Dent Repair ${city} — No Paint | Dent Master`,
    metaDescription: fitMetaDescription(
      `${primaryKeyword} in ${cs} for door dings, hail dents, and minor damage. We protect factory paint, keep pricing clear, and quote from photos.`
    ),
    h1: `Restore Your Car With Dent Repair ${city} — No Paint`,
    h1Es: `Reparación de Abolladuras sin Pintura en ${city}, FL`,
    intro: `${primaryKeyword} in ${cs} helps you fix dents without repainting. We protect your factory finish and keep the process simple. Send photos to get a fast quote.`,
    ...
    faqs: [
      ...
      { q: `What's the best PDR shop in ${city} Florida?`, a: `Dent Master Franchise is a trusted paintless dent repair team serving ${city} and the surrounding area. We offer mobile service when conditions allow, work with insurance, and stand behind our repairs.` },
      { q: `Can you remove door dings in ${city} parking lots?`, a: `Yes. We remove parking lot door dings throughout ${city} using paintless dent repair when the paint is intact. Many door ding repairs can be completed quickly, and we’ll confirm timing after a photo review.` },
      { q: `Do you offer mobile dent repair in ${city} FL?`, a: `Yes. Our mobile PDR service covers ${city} and many surrounding areas. We can often come to your home, office, or another convenient location when conditions support a clean repair.` },
      { q: `How much does hail damage repair cost in ${city}?`, a: `Hail damage pricing depends on the number of dents, panel access, and severity. Send photos for a fast estimate, and we can help you understand what insurance may cover.` },
      { q: `Can PDR fix creases and sharp dents in ${city}?`, a: `Yes, our certified technicians can repair most creases and sharp dents using advanced PDR techniques. Severe creases may require traditional body work - we'll assess and recommend the best approach.` }
    ],
    faqsEs: [
