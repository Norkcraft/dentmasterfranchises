...
function CitySlider({ before, after, label }: { before: string; after: string; label: string }) {
  ...
  return (
    <div className="card-elevated p-0 overflow-hidden">
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-muted-foreground mt-1">Drag to compare — real results, no repainting.</p>
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <article className="section-padding bg-background">
        <div className="section-container max-w-4xl" ref={sectionsRef}>
          ...
          <div className="mb-12">
            <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6 font-heading">See Our Work in {city.city}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <CitySlider before={beforeDoor} after={afterDoor} label={`Door Dent Repair in ${city.city}`} />
              <CitySlider before={beforeHood} after={afterHood} label={`Hood Dent Repair in ${city.city}`} />
            </div>
